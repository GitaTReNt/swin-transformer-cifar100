...
from swin_transformer import swin_t  # 新的 Swin Transformer 模型
...
def main(pretrained=None):
    ...
    model = swin_t(hidden_dim=96, layers=(2, 2, 6, 2), heads=(3, 6, 12, 24), num_classes=1000).to(device)  # 使用新的 Swin Transformer 模型
    if pretrained:
    ...
    num_epochs = 90  # 通常在 ImageNet 上训练 90 个 epoch
    best_val_acc = 0
